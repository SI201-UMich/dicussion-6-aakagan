...
class PollReader():
    ...
    def likely_voter_polling_average(self):
        ...
        for i in range(len(self.data_dict['sample type'])):
            if self.data_dict['sample type'][i] == 'LV':
                harrisSum += self.data_dict['Harris result'][i]
                trumpSum += self.data_dict['Trump result'][i]
                count += 1
        if count > 0:
            return (harrisSum / count, trumpSum / count)
        else:
            return (0.0, 0.0)
    ...
    def polling_history_change(self):
        ...
        """
        firstHarris = sum(self.data_dict['Harris result'][-30:]) / 30
        firstTrump = sum(self.data_dict['Trump result'][-30:]) / 30
        lastHarris = sum(self.data_dict['Harris result'][:30]) / 30
        lastTrump = sum(self.data_dict['Trump result'][:30]) / 30
